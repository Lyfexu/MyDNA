...
public aspect ProfilerAspects {
	...
	//
	// pointcut seriesSingleRunGeneration(Series s, int run, int
	// numberOfBatches) : execution(* SeriesGeneration.generateRun(Series, int,
	// int, ..)) && args(s, run, numberOfBatches, ..);
	// pointcut batchGenerationCallee(Series s) : (execution(*
	// SeriesGeneration.generateInitialData(Series)) || execution(*
	// SeriesGeneration.generateNextBatch(Series))) && args(s);
	// pointcut seriesGeneration() : execution(* SeriesGeneration.generate(..));
	//
	// pointcut graphGeneration(IGraphGenerator graphGenerator) : execution(*
	// IGraphGenerator+.generate()) && target(graphGenerator);
	// pointcut graphGenerated(): cflow(graphGeneration(*));
	//
	// pointcut batchGeneration(BatchGenerator batchGenerator) : execution(*
	// BatchGenerator+.generate(*)) && target(batchGenerator);
	// pointcut batchGenerated() : cflow(batchGeneration(*));
	//
	// pointcut initialMetric(Metric metricObject) : execution(*
	// Metric+.compute()) && target(metricObject);
	// pointcut metricAppliedOnUpdate(Metric metricObject, Update updateObject)
	// : (execution(* Metric+.applyBeforeUpdate(Update+))
	// || execution(* Metric+.applyAfterUpdate(Update+))) && args(updateObject)
	// && target(metricObject);
	// pointcut metricAppliedOnBatch(Metric metricObject, Batch batchObject) :
	// (execution(* Metric+.applyBeforeBatch(Batch+))
	// || execution(* Metric+.applyAfterBatch(Batch+))) && args(batchObject) &&
	// target(metricObject);
	// pointcut metricApplied() : cflow(initialMetric(*)) ||
	// cflow(metricAppliedOnUpdate(*, *)) || cflow(metricAppliedOnBatch(*, *));
	//
	// pointcut updateApplication(Update updateObject) : execution(*
	// Update+.apply(*)) && target(updateObject);
	// pointcut updateApplied(): cflow(updateApplication(*));
	//
	// pointcut watchedCall() : graphGenerated() || batchGenerated() ||
	// metricApplied() || updateApplied();
	//
	// pointcut initGDS(Graph g, GraphDataStructure gds) : this(g) &&
	// execution(Graph+.new(String,long, GraphDataStructure,..)) &&
	// args(*,*,gds,..);
	//
	// pointcut init(DataStructure list, boolean firstTime) : call(*
	// IDataStructure+.init(Class, int, boolean)) && target(list) && args(*,*,
	// firstTime) && watchedCall();
	// pointcut add(DataStructure list) : call(* IDataStructure+.add(..)) &&
	// target(list) && watchedCall();
	// pointcut remove(DataStructure list) : call(* IDataStructure+.remove(..))
	// && target(list) && watchedCall();
	// pointcut contains(DataStructure list) : call(*
	// IDataStructure+.contains(..)) && target(list) && watchedCall();
	// pointcut getElement(DataStructure list) : call(* IDataStructure+.get(..))
	// && target(list) && watchedCall();
	// pointcut size(DataStructure list) : call(* IDataStructure+.size()) &&
	// target(list) && watchedCall();
	// pointcut random(DataStructure list) : call(* IDataStructure+.getRandom())
	// && target(list) && watchedCall();
	// pointcut iterator(DataStructure list) : execution(*
	// DataStructure+.iterator()) && target(list) && watchedCall();
	//
	// before(Series s, int run, int numberOfBatches) :
	// seriesSingleRunGeneration(s, run, numberOfBatches) {
	// Profiler.setSeriesData(s, numberOfBatches);
	// Profiler.startRun(run);
	// HotSwap.reset();
	// }
	//
	// after(Series s, int run, int numberOfBatches) :
	// seriesSingleRunGeneration(s, run, numberOfBatches) {
	// try {
	// Profiler.finishRun();
	// } catch (IOException e) {
	...
	// }
	//
	// Graph around(IGraphGenerator graphGenerator) :
	...
	// }
	//
	// Batch around(BatchGenerator batchGenerator) :
	// batchGeneration(batchGenerator) {
	// formerCountKey.push(currentCountKey);
	...
	// }
	//
	// boolean around(Metric metricObject, Update updateObject) :
	// metricAppliedOnUpdate(metricObject, updateObject) {
	// formerCountKey.push(currentCountKey);
	// currentCountKey = metricObject.getName();
	// Profiler.addMetricName(currentCountKey);
	// Profiler.setInInitialBatch(false);
	...
	// }
	//
	// boolean around(Metric metricObject, Batch batchObject) :
	// metricAppliedOnBatch(metricObject, batchObject) {
	// formerCountKey.push(currentCountKey);
	// currentCountKey = metricObject.getName();
	// Profiler.addMetricName(currentCountKey);
	// Profiler.setInInitialBatch(false);
