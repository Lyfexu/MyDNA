 ...
 *
 */
public class MotifTraverser3 extends Traverser2 {
	...
	private static List<Path> foundSubgraphs;
	...
	private static void updateHubsWithRemovedEdge() {
		for(StoredPathInfo hubToUpdate : hubsToUpdate) {
			//StoredPathConverter.removePathFromHub(actEdge, hubToUpdate.getStoredPathRoot(), maxSize);
			RedundantHubExplorer.removePath(actEdge, hubToUpdate.getStoredPathRoot());
		}
	}

	private static void updateHubsWithAddedEdge() {
		Collections.sort(foundSubgraphs, new Comparator<Path>() {
			@Override
		...
		for(StoredPathInfo hubToUpdate : hubsToUpdate) {
			for(Path foundSubgraph : foundSubgraphs) {
				if(foundSubgraph.getGraph().getNodes().contains(hubToUpdate.getAssociatedNode())) {
					//StoredPathConverter.incorporatePathIntoHub(actEdge, foundSubgraph.getGraph(), hubToUpdate.getStoredPathRoot());
